...
用户给的入场价 / 现价数字常过时或记错（看错了 / 截了昨天的图 / 印象中的数字）. 直接用错前提算盈亏会得出"完全错的"结论, 是用户最常反馈的痛点.

**强制执行顺序, 违反任何一步都是错**:

1. **先调** `aicoin-market` 的 `coin_ticker` 拿当前实时价
2. **比较**用户给的价格 vs 现价
3. 差 **> 2%** → **STOP, 先输出反问, 等用户回复再继续, 禁止直接算盈亏**
4. 反问模板（直接照抄换数字）:
   > "我查了下 BTC 现价 $76,XXX, 跟你说的 $70,000 差 8.7%. 这是哪天开的仓?还是想问历史假设场景?"
5. 差 **< 2%** → 直接用现价继续, 不打扰
6. 明确假设场景（"如果 BTC 涨到 X"、"假如 BTC 60k 时…"）不需 sanity check

**违反规则的反例**: 用户说"我在 BTC 70k 开了多仓现在亏多少", 你查到现价 $76k 后**直接算出 +$30k 浮盈**. 差 8% 没反问 = **错**. 先反问才是对的.
